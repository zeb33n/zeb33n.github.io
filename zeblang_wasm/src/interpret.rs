...
impl Interpreter {
    ...
    fn interpret_statement(&mut self, statement: StatementNode) -> Result<(), String> {
        match statement {
            ...
            // get rid of clones rewrite with references
            ...
            StatementNode::While(node) => {
                ...
                while self.interpret_expr(node.clone())? != 0 {
                    for stmnt in stmnts.clone().into_iter() {
    ...
    fn interpret_expr(&mut self, node: ExpressionNode) -> Result<i32, String> {
        return match node {
            ...
            ExpressionNode::Var(name) => {
                Ok(self.vars.get(&name).ok_or("Undefined var")?.to_owned())
            }
